...
export const issueCredential = async (req: Request, res: Response) => {
  try {
    ...
    if (existing) {
      ...
      if (existingCredential.name === name) {
        return res.status(400).json({
          ...
          error: "Credential already exists for this name and email",
      ...
      return res.status(400).json({
        ...
        error: "A credential with this email already exists under a different name",
        credential:existingCredential,
    ...
    return res.status(201).json({
      status: "issued",
      message: `Credential issued by worker-${worker}`,
      credential,
      worker,
    });
  } catch (error) {
